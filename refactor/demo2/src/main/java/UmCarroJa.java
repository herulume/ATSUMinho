...
public class UmCarroJa implements Serializable {
    ...
    public List<Aluguer> alugueresClassificarCliente() throws NaoExistemAlugueresException {
        ...
        for (List<Aluguer> lAlugs : alugsProp.values()) {
            for (Aluguer alug : lAlugs) {
                verificarSeAluguerEClassificavel(alugsClassif, alug);
            }
        }
        return alugsClassif;
    }

    private void verificarSeAluguerEClassificavel(List<Aluguer> alugsClassif, Aluguer alug) {
    ...
    public void classificarCliente(Aluguer alug, int classif) {
        ...
        classificacaoFinal = calcularNovaClassificacao(classif, cli);

        cli.setClassificacao(classificacaoFinal);
    ...
    private int calcularNovaClassificacao(int classif, Cliente cli) {
        double novaClassificacao;
        Long classificacaoArredondada;

        novaClassificacao = (cli.getClassificacao() + classif) / (cli.getNAlugueres());
        classificacaoArredondada = Math.round(novaClassificacao);

        return classificacaoArredondada.intValue();
    }
    ...
    public List<Aluguer> determinarListaAlugCli(String matricula, String mail) throws VeiculoNaoESeuException, UtilizadorNaoExisteException {
        ...
        for (Aluguer aluguer : this.alugueres.get(user.getNIF()).get(matricula)) {
            Aluguer clone = aluguer.clone();
            alugs.add(clone);
        }
    ...
    private List<Veiculo> disponiveisAlugar(Coordinate destino, ParDatas date) {
        ...
        for (Veiculo v : this.veiculos.values()) {
            if (v.getDisponibilidade()) {
                long count = 0L;
                for (ParDatas d : v.getDatasAlugueres()) {
                    if (!d.isAvailable(date)) {
                        count++;
                    }
                }
                if (count == 0
                        && autonomiaSuf(v, destino)) {
                    Veiculo clone = v.clone();
                    list.add(clone);
                }
            }
        }
    ...
    public List<Veiculo> maisProximo(Coordinate destino, ParDatas date, int quantos) throws NaoExistemVeiculosDisponiveisException {
        ...
        List<Veiculo> list = new ArrayList<>();
        long limit = quantos;
        for (Veiculo veiculosOrdenado : veiculosOrdenados) {
            if (limit-- == 0) break;
            list.add(veiculosOrdenado);
        }
        return list;
    }
    ...
    public List<Veiculo> maisBaratoNoPerimetro(Coordinate destino, Coordinate cordsCli, ParDatas date, double perimetro, int quantos) throws NaoExistemVeiculosDisponiveisException {
        ...
        List<Veiculo> list = new ArrayList<>();
        long limit = quantos;
        for (Veiculo veiculo : veiculosBaratosNoP) {
            if (limit-- == 0) break;
            list.add(veiculo);
        }
        return list;
    ...
    public Veiculo veiculoEspecifico(Coordinate destino, ParDatas date, String id) throws VeiculoNaoExisteException, VeiculoIndisponivelException {
        ...
        if (this.veiculos.get(id).getDisponibilidade() && autonomiaSuf(this.veiculos.get(id), destino) &&
                datasDeUmVeiculo.stream().allMatch(d -> d.isAvailable(date))) {
    ...
    public List<Veiculo> determinadaAutonomia(Coordinate destino, ParDatas date, int val1, int val2, int quantos) throws NaoExistemVeiculosDisponiveisException {
        ...
        List<Veiculo> list = new ArrayList<>();
        long limit = quantos;
        for (Veiculo veiculo : veiculosAuto) {
            if (limit-- == 0) break;
            list.add(veiculo);
        }
        return list;
